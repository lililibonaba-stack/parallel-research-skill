# Free LLM Leaderboard

免费大语言模型推荐榜单，持续收集可免费调用的模型，并从稳定性、调用限制、上下文长度、参数规模和适用场景等维度进行整理。
...
以下顺序是当前的推荐顺序，不代表经过统一基准测试得出的固定名次。序号仅用于引用列表项。
...
稳定性分数为当前主观初评，不代表统一基准测试结果。后续实测数据补充后会调整。
...
当前榜单按 cheapapis 页面记录为 `20 RPM`。cheapapis 的每日额度、并发限制和不同模型的实际成功率仍需后续测试确认，因此暂不填写 RPD 和并发数。
